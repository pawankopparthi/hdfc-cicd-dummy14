...
var environment = config['hdfc-cicd-dummy14'].env;
...
console.log('&$hdfc-cicd-dummy14 api: [' +  domains[environment] + ', ' + config['hdfc-cicd-dummy14'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['hdfc-cicd-dummy14'].app, config['hdfc-cicd-dummy14'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['hdfc-cicd-dummy14'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
